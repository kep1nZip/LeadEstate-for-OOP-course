...
<%
    ...
    /* ── Format untuk followupDate ── */
    java.text.SimpleDateFormat sdfWaktu  = new java.text.SimpleDateFormat("HH:mm", locale);
    java.text.SimpleDateFormat sdfLengkap = new java.text.SimpleDateFormat("d MMM yyyy, HH:mm", locale);
%>
            ...
            <div class="flex items-center gap-2.5 px-3.5 py-2.5 rounded-[10px] text-[13.5px] font-bold cursor-default"
                ...
                <span class="w-4.5 h-4.5 flex items-center justify-center flex-shrink-0">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#0b1622" stroke-width="1.8">
                        ...
                        <polygon points="10,8 16,12 10,16" fill="#0b1622"/>
                    </svg>
                </span>
                Reminder &amp; Follow-Up
                <c:if test="${jumlahNotif > 0}">
                    <span class="ml-auto text-[10px] font-bold min-w-[18px] h-[18px] px-1.5 rounded-full flex items-center justify-center"
                          style="background:var(--danger); color:#fff;">${jumlahNotif}</span>
                </c:if>
            </div>
    ...
    <main class="flex-1 flex flex-col overflow-hidden">
        ...
        <div class="flex items-center justify-between flex-shrink-0 px-7"
            ...
            <div class="flex items-center gap-3.5">
                ...
                <a href="${pageContext.request.contextPath}/reminder?action=cekOverdue"
                   title="Cek & update reminder overdue"
        ...
        <div class="flex flex-1 overflow-hidden">
            ...
            <div class="w-[380px] flex-shrink-0 flex flex-col overflow-hidden"
                ...
                <!-- Filter Tabs -->
                <div class="flex gap-1.5 p-3.5 flex-shrink-0"
                    ...
                    %>
                    <a href="${pageContext.request.contextPath}/reminder?tab=<%= tabKeys[i] %>"
                       class="flex-1 text-center py-2 rounded-lg text-xs font-medium no-underline transition-all"
                       style="<%= isActive
                           ? "background:var(--brand-gold); color:#0b1622; font-weight:700; border:1px solid var(--brand-gold);"
                           : "color:var(--text-secondary); border:1px solid transparent;" %>"
                       onmouseover="<%= !isActive ? "this.style.background='var(--content-bg)';" : "" %>"
                ...
                <div class="px-4 py-2.5 flex-shrink-0" style="border-bottom:1px solid var(--border);">
                    <div class="relative">
                        <svg class="absolute left-2.5 top-1/2 -translate-y-1/2" width="14" height="14"
                             viewBox="0 0 24 24" fill="currentColor" style="color:var(--text-muted);">
                            <path d="M15.5 14h-.79l-.28-.27A6.47 6.47 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/>
                        </svg>
                        <input id="searchInput" type="text" placeholder="Cari nama atau properti..."
                               class="w-full rounded-lg pl-8 pr-3 py-2 text-[13px] outline-none transition-all"
                               style="border:1px solid var(--border); background:var(--content-bg); color:var(--text-primary); font-family:'DM Sans',sans-serif;">
                    </div>
                </div>
                ...
                <div id="leadList" class="overflow-y-auto flex-1 scroll-thin">
                    ...
                    <c:if test="${tabAktif == 'semua' || tabAktif == null}">
                        <c:if test="${empty followUpHariIni && empty followUpTertunda && empty followUpSelesai}">
                            <div class="flex flex-col items-center justify-center gap-2.5 p-10" style="color:var(--text-muted);">
                                ...
                                <p class="text-sm text-center">Tidak ada follow-up hari ini</p>
                            </div>
                        </c:if>
                        <c:forEach var="fu" items="${followUpHariIni}">
                            <jsp:include page="/WEB-INF/jsp/reminder/lead-card.jsp">
                                <jsp:param name="fuId"      value="${fu.id}"/>
                                <jsp:param name="fuLeadId"  value="${fu.leadId}"/>
                                <jsp:param name="fuNotes"   value="${fu.notes}"/>
                                <jsp:param name="fuStatus"  value="${fu.status}"/>
                                <jsp:param name="fuDate"    value="${fu.followupDate}"/>
                                <jsp:param name="tag"       value="today"/>
                            </jsp:include>
                        </c:forEach>
                    ...
                    <c:if test="${tabAktif == 'hariIni'}">
                        <c:choose>
                            ...
                            <c:otherwise>
                                <c:forEach var="fu" items="${followUpHariIni}">
                                    <%-- Render inline karena tidak bisa nested include dengan EL --%>
                                    <div class="lead-card-item flex items-center gap-3 px-4 py-3 cursor-pointer transition-all ${leadIdDipilih == fu.leadId ? 'selected' : ''}"
                                        ...
                                        <div class="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-white flex-shrink-0"
                                             style="background:${fu.leadId % 5 == 0 ? '#f59e0b' : fu.leadId % 5 == 1 ? '#6366f1' : fu.leadId % 5 == 2 ? '#10b981' : fu.leadId % 5 == 3 ? '#ef4444' : '#8b5cf6'};">
                                            FU
                                        </div>
                                        <div class="flex-1 min-w-0">
                                            <div class="text-[13px] font-semibold truncate">Lead #${fu.leadId}</div>
                                            <div class="text-[11px] mt-px truncate" style="color:var(--text-muted);">
                                                <fmt:formatDate value="${fu.followupDate}" pattern="HH:mm"/>
                                            </div>
                            ...
                            <c:otherwise>
                                <c:forEach var="fu" items="${followUpTertunda}">
                                    <div class="lead-card-item flex items-center gap-3 px-4 py-3 cursor-pointer transition-all ${leadIdDipilih == fu.leadId ? 'selected' : ''}"
                                         ...
                                         onclick="pilihLead(${fu.leadId}, ${fu.id})">
                                        <div class="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-white flex-shrink-0"
                                             style="background:#ef4444;">FU</div>
                                        <div class="flex-1 min-w-0">
                                            <div class="text-[13px] font-semibold truncate">Lead #${fu.leadId}</div>
                                            <div class="text-[11px] mt-px truncate" style="color:var(--text-muted);">
                                                <fmt:formatDate value="${fu.followupDate}" pattern="d MMM yyyy, HH:mm"/>
                                            </div>
                                        </div>
                                        <div class="text-right flex-shrink-0">
                    ...
                    <c:if test="${tabAktif == 'selesai'}">
                        <c:choose>
                            ...
                            <c:otherwise>
                                <c:forEach var="fu" items="${followUpSelesai}">
                                    <div class="lead-card-item flex items-center gap-3 px-4 py-3 cursor-pointer transition-all"
                                         style="border-bottom:1px solid #f3f4f6; opacity:.7;"
                                         onclick="pilihLead(${fu.leadId}, ${fu.id})">
                                        <div class="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-white flex-shrink-0"
                                             style="background:#6366f1;">FU</div>
                                        <div class="flex-1 min-w-0">
                                            <div class="text-[13px] font-semibold truncate">Lead #${fu.leadId}</div>
                                            <div class="text-[11px] mt-px truncate" style="color:var(--text-muted);">
                                                <fmt:formatDate value="${fu.followupDate}" pattern="d MMM yyyy, HH:mm"/>
            ...
            <div class="flex-1 flex flex-col overflow-hidden" style="background:var(--content-bg);">
                <div id="rightPanel" class="flex-1 overflow-y-auto scroll-thin p-5 flex flex-col gap-4">
                    ...
                    <c:choose>
                        ...
                        <%-- ── DETAIL PANEL (ada lead yang dipilih) ── --%>
                        <c:otherwise>
                            ...
                            <div class="rounded-2xl p-5" style="background:var(--card-bg); border:1px solid var(--border);">
                                <div class="flex items-center gap-3.5 mb-4">
                                    ...
                                    <div class="flex-1">
                                        <div class="text-lg font-bold">Lead #${leadIdDipilih}</div>
                                        <div class="text-[13px] mt-0.5" style="color:var(--text-muted);">
                                            Klik "WhatsApp" untuk menghubungi langsung
                                        </div>
                                        <div class="flex gap-2 mt-2">
                                            <button onclick="bukaWhatsApp()"
                                                    class="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-semibold cursor-pointer transition-all"
                                    ...
                                    <div class="text-right">
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold sp-proses">
                                            Sedang Diproses
                                        </span>
                                        ...
                                        <div class="text-[13px] font-medium" style="color:var(--text-primary);">
                                            <c:choose>
                                                <c:when test="${not empty riwayatFollowUp}">
                                                    <fmt:formatDate value="${riwayatFollowUp[0].followupDate}" pattern="d MMM yyyy, HH:mm"/>
                                                </c:when>
                            ...
                            <%-- RIWAYAT FOLLOW-UP TIMELINE ── --%>
                            <div class="rounded-2xl p-5" style="background:var(--card-bg); border:1px solid var(--border);">
                                <div class="text-[13px] font-bold mb-3 flex items-center gap-1.5">
                                    🕐 Riwayat Follow Up
                                </div>
                                <c:choose>
                                                ...
                                                <div class="flex gap-3">
                                                    ...
                                                    <div class="flex flex-col items-center">
                                                        <div class="w-2.5 h-2.5 rounded-full flex-shrink-0 mt-1"
                                                             style="background:${fu.status == 'Selesai' ? 'var(--success)' : 'var(--brand-gold)'};">
                                                        </div>
                                                        <c:if test="${!loop.last}">
                            ...
                            <div class="rounded-2xl p-5" style="background:var(--card-bg); border:1px solid var(--border);">
                                ...
                                <!-- WhatsApp Box -->
                                <div class="rounded-xl p-3.5 mb-3.5"
                                     style="background:#f0fdf4; border:1px solid #bbf7d0;">
                                    <div class="text-[11px] font-semibold mb-2" style="color:#16a34a;">
                                        💬 Kirim via WhatsApp
                                    </div>
                                    <!-- Template Chips -->
                                    <div class="flex flex-col gap-1.5">
                                        <div class="wa-chip px-3 py-1.5 rounded-lg text-xs cursor-pointer"
                                             style="border:1px solid #bbf7d0; background:#fff; color:#15803d; font-family:'DM Sans',sans-serif;"
                                             onclick="pilihTemplate('reminder')">
                                            📅 Reminder Jadwal
                                        </div>
                                        <div class="wa-chip px-3 py-1.5 rounded-lg text-xs cursor-pointer"
                                             style="border:1px solid #bbf7d0; background:#fff; color:#15803d; font-family:'DM Sans',sans-serif;"
                                             onclick="pilihTemplate('info')">
                                            🏠 Info Properti
                                        </div>
                                        <div class="wa-chip px-3 py-1.5 rounded-lg text-xs cursor-pointer"
                                             style="border:1px solid #bbf7d0; background:#fff; color:#15803d; font-family:'DM Sans',sans-serif;"
                                             onclick="pilihTemplate('harga')">
                                            💰 Penawaran Harga
                                        </div>
                                    </div>
                                    <!-- Preview -->
                                    <div id="waPreview" class="rounded-lg px-3 py-2.5 mt-2.5 text-xs leading-relaxed min-h-[60px]"
                                         style="background:var(--content-bg); border:1px dashed var(--border); color:var(--text-secondary);">
                                        Pilih template pesan di atas...
                                    </div>
                                    <button onclick="kirimWhatsApp()"
                                            class="w-full mt-2.5 py-2.5 rounded-lg text-[13px] font-bold flex items-center justify-center gap-2 transition-all"
                                            style="background:#16a34a; color:#fff; border:none; cursor:pointer; font-family:'DM Sans',sans-serif;"
                                ...
                                <form method="post" action="${pageContext.request.contextPath}/followup">
                                    ...
                                    <input type="hidden" name="followupDate" id="followupDateInput"
                                           value="<%= new java.text.SimpleDateFormat("yyyy-MM-dd'T'HH:mm").format(new java.util.Date()) %>">

                                    <textarea id="notesArea" name="notes" rows="3"
                                              placeholder="Tulis catatan hasil follow up... (misal: Lead sudah dihubungi, respon positif, minta brosur)"
                                              class="w-full rounded-lg px-3 py-2.5 text-[13px] outline-none resize-none transition-all"
                                              style="border:1px solid var(--border); background:var(--content-bg); color:var(--text-primary); font-family:'DM Sans',sans-serif; min-height:70px;"></textarea>

                                    <div class="flex gap-2 mt-2.5">
                                        <!-- Tandai Selesai (pakai form terpisah via JS) -->
                                        <button type="button" onclick="tandaiSelesai()"
                                                class="flex-1 py-2.5 rounded-lg text-xs font-bold flex items-center justify-center gap-1.5 transition-all"
                                                style="background:var(--brand-gold); color:#0b1622; border:none; cursor:pointer; font-family:'DM Sans',sans-serif;"
                                                onmouseover="this.style.background='#b8963d';"
                                        ...
                                        <button type="submit"
                                                class="flex-1 py-2.5 rounded-lg text-xs font-bold flex items-center justify-center gap-1.5 transition-all"
                                                style="background:var(--content-bg); color:var(--text-secondary); border:1px solid var(--border); cursor:pointer; font-family:'DM Sans',sans-serif;"
                                                onmouseover="this.style.background='#e5e7eb';"
                                                onmouseout="this.style.background='var(--content-bg)';">
                                            📝 Catat Aktivitas
                                        </button>
                                    </div>
                                </form>
                                ...
                                <form id="formSelesai" method="post"
                                      action="${pageContext.request.contextPath}/followup" style="display:none;">
...
<c:if test="${param.sukses != null}">
    <div id="toastSukses" class="fixed bottom-5 right-5 z-50 px-5 py-3 rounded-xl text-sm font-semibold shadow-lg"
        ...
        ✅
        <c:choose>
            <c:when test="${param.sukses == 'catat'}">Aktivitas berhasil dicatat!</c:when>
            <c:when test="${param.sukses == 'selesai'}">Follow-up ditandai selesai!</c:when>
            <c:when test="${param.sukses == 'batalkan'}">Reminder berhasil dibatalkan.</c:when>
            <c:otherwise>Berhasil!</c:otherwise>
        </c:choose>
    </div>
    <script>setTimeout(() => document.getElementById('toastSukses')?.remove(), 3500);</script>
</c:if>
<c:if test="${param.gagal != null}">
    <div id="toastGagal" class="fixed bottom-5 right-5 z-50 px-5 py-3 rounded-xl text-sm font-semibold shadow-lg"
...
<script>
    ...
    // ── Hitung jumlah item di list untuk badge ──
    ...
    document.getElementById('searchInput').addEventListener('input', function () {
        ...
        // Update count
        const visible = [...document.querySelectorAll('.lead-card-item')]
                        .filter(c => c.style.display !== 'none').length;
        document.getElementById('leadCount').textContent = visible + ' lead';
    });

    // ── Pilih lead → redirect ke halaman yang sama dengan leadId ──
    ...
    function kirimWhatsApp() {
        ...
        // Nomor HP Lead — idealnya dikirim Controller via request attribute
        // Fallback: buka WA tanpa nomor (akan dibuka ke wa.me saja)
        const phone = ''; // TODO: isi dari ${lead.phone} jika Controller menyediakan objek Lead
    ...
    // ── Tandai Selesai → submit form tersembunyi ──
    ...
    // ── Update followupDate ke waktu sekarang sebelum submit ──
